...
export default function Hero({ data }) {
  ...
  return (
    <div >
      ...
      <div className='flex flex-col gap-y-5 gap-x-4 mb-9 items-start md:flex-row md:md-container'>

        <div className='md:w-1/2 md:order-2'>
          <BigNewsCard imgLink={data?.mainNews.image} text={data.mainNews.title} description={data?.mainNews.description} />
        </div>

        <div className='container flex gap-4 *:flex-1 md:flex-1 md:order-1 md:flex-col'>
          {data.sideNews.map((item, i) => (
            <SmallNewsCard key={i} imgLink={item.image} text={item.title} />
          ))}
        </div>
        ...
        <div className='container flex-1 md:order-3'>
          ...
          {data.topHeadlines.map((item, i) => (
            <Link key={i} href='#' className='border-b-[#e0e0e0] not-last:border-b block hover:text-(--main-color) transition-none pb-2 pt-0.5 md:pb-3 md:pt-1'>
              <h3 className='font-medium text-[14px] line-clamp-3 '>{item.title}</h3>
            </Link>
          ))}
